...
base_dir = '/home/data'
bids_dir = os.path.join(base_dir, AG, exp) #level 0
wf_dir = os.path.join(bids_dir, 'workflows') #level 1
rawdata_dir = os.path.join(bids_dir, 'rawdata') #level 1
source_dir = os.path.join(bids_dir, 'sourcedata') #level 1
derivatives_dir = os.path.join(bids_dir, 'derivatives') #level 1
fmriprep_dir = os.path.join(derivatives_dir, 'fmriprep') #level 1
freesurfer_dir = os.path.join(derivatives_dir, 'freesurfer') #level 1
ROIs_dir = os.path.join(derivatives_dir, 'ROIs') #level 1
DTI_dir = os.path.join(derivatives_dir, 'DTI') #level 1
group_DTI_dir = os.path.join(DTI_dir, 'group')
if not os.path.exists(group_DTI_dir): 
    os.makedirs(group_DTI_dir)
crashdir = os.path.join(bids_dir, 'crashfiles') #level 1
...
def fmriprep_files(derivatives_dir, ptc):
    ...
    # mask_MNI152NLin2009cAsym = glob.glob(os.path.join(p_fmriprep,'**/*space-MNI152NLin2009cAsym_desc-brain_mask.nii.gz'),recursive=True)[0]
...
# gibbs_AP =  Node(mrtrix.MRDeGibbs(nthreads=n_threads), name='gibbs_AP')
# gibbs_PA =  Node(mrtrix.MRDeGibbs(nthreads=n_threads), name='gibbs_PE')
...
##topup --imain=AP_PA_b0 --datain=acqparams.txt --config=b02b0.cnf --out=topup_AP_PA_b0 --iout=my_hifi_b0 --fout=topup_AP_PA_b0_fout
...
# IMPORTANT: define out_file for DWIBiasCorrect, because for some subjects nipype threw an error when not defined
bias_correct = Node(mrtrix.DWIBiasCorrect(nthreads=n_threads, use_fsl=True,
                                          out_file='eddy_biascorr.nii.gz'), name='bias_correct')

#eddy --imain=dwidata --mask=my_hifi_brain --indix=index.txt --acqp=acqparams.txt --bvecs=bvecs --bvals=bvals --fwhm=0 --topup=topup_AP_PA_b0 --flm=quadratic --out=eddy_unwarped_images
...
#bbreg.inputs.args = '--lta reg_FA2FS.lta' 
...
# FA2MNI_warp.inputs.reference = os.path.join(os.getenv('FSLDIR'),'data/standard/MNI152_T1_1mm_brain.nii.gz') 
...
def get_ROIs(ptc, ROIs_dir, space, rois_vol=[], rois_surf=[]):
    ...
    with open(seed_file,'w') as outfile:
        outfile.write('\n'.join(ROIs))
        outfile.write('\n') # very important, otherwise FSL will not include the last ROI when computing the network matrix
...
dict_tract = os.path.join(base_dir, 'dict_tractography.txt')
...
# probtrack_fs = Node(probtrackFS(options=options), mem_gb=150, name='probtrack_fs_corr')
...
def get_waytotals(probtrack_outd):
    ...
    # d = {'participant':'sub-'+sub, 'seed':'_'.join([seedh,seed]), 'target':'_'.join([targeth,target]), 'streamlines':waytotal}
