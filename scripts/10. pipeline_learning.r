...
base_dir = file.path("E:/data")
fig_dir =  file.path("E:/data/figures")
learning_dir =  file.path(base_dir, 'learning')
...
df_stats_AC_short = df_stats_AC %>% ungroup %>% group_by(participant,AG,study,task) %>% 
...
m = lmer(amplitude ~ cond2*trial_cond2 + (1|participant), data=df_stats_AC_short)
summary(m)
...
write.table(df_all, file.path(learning_dir, "stat-bf_desc-EDA_df.csv"), row.names=FALSE)
...
AG = "A08"
dat = read.csv(file.path(base_dir, AG, "desc-behaviour_df.tsv"), sep='\t')
...
write.table(df_all, file.path(learning_dir, "stat-logit_desc-beh_df.tsv"), row.names=FALSE, sep="\t")
