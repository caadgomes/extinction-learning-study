...
bids_dir = os.path.join('/home/data',AG,exp)

wf_dir = os.path.join(bids_dir, 'workflows') #level 1
source_dir = os.path.join(bids_dir, 'sourcedata') #level 1
rawdata_dir = os.path.join(bids_dir, 'rawdata') #level 2 - data
derivatives_dir = os.path.join(bids_dir, 'derivatives') #level 2 - data
func_denoised_dir = os.path.join(derivatives_dir, 'func_denoised') #level 2 - data
...
selected_rois = {
        ...
        'Bilateral_vmPFC':
            {'Left_vmPFC':['ctx-lh-medialorbitofrontal'], #['ctx-lh-lateralorbitofrontal','ctx-lh-medialorbitofrontal'], 
             'Right_vmPFC':['ctx-rh-medialorbitofrontal']}, #['ctx-rh-lateralorbitofrontal','ctx-rh-medialorbitofrontal']},
        'Bilateral_dACC':
            {'Left_dACC':['ctx-lh-caudalanteriorcingulate'], #['ctx-lh-rostralanteriorcingulate', 'ctx-lh-caudalanteriorcingulate'], 
             'Right_dACC':['ctx-rh-caudalanteriorcingulate']}, #['ctx-rh-rostralanteriorcingulate', 'ctx-rh-caudalanteriorcingulate']}
        'Bilateral_Thalamus':
...
def all_atlases(ptc, derivatives_dir, pat):
    import os, glob
    p_fmriprep = os.path.join(derivatives_dir,'fmriprep',ptc) #don't change the name of this subfolder (fmriprep). It's the one fmriprep creates
...
# Annotation to Label Conversion
...
class Annotation2Label(FSCommand):
    ...
    def _format_arg(self, name, spec, value):
        ...
        # when you call Annotation2Label you give it an annot_file which can start with lh.*
        # or .rh*. You need to get pass the hemisphere to mri_annotation2label, so this will
        # extract it
        elif name=='hemi':
...
# merge_FS_labels = Node(Function(function=merge_labels, input_names=['ptc','p_rois','labels','df'], 
...
def get_tissues(ptc, aparc_aseg, fs_file, tissues, p_rois):
    ...
    for k,v in ind_array.items():
        ...
        l = '{ptc}_space-fs_roi-{region}_hemi-Bilateral_desc-aparcaseg_mask.nii.gz'.format(ptc=ptc,
                                                                                           region=k)
        filename = os.path.join(p_rois,'freesurfer',l)
        # filename = os.path.abspath('%s_mask.nii.gz'%k)
...
wf_create_ROIs.connect([\
                        ...
                        # (gunzip_anat, datasink, [('out_file','cerebellum')]),
                        # # (acpcdetect, datasink, [('out_file','cerebellum.@f1'),
                        # #                         ('fsl_mat','cerebellum.@f2')]),
                        ])
