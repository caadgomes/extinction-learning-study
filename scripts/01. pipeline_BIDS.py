...
base_dir = os.path.join('/home/data') #level 0
    ...
    for file in files:
        ...
        if re.search(anat_pat, pathfile.stem) is not None: #if *Anat*.json
            ...
            if ext=='.json':
                ...
                with open(pathfile, 'r+') as json_file:
                    data = json.load(json_file) 
                    if AG not in ['A05', 'A12']:
                        WFS = dicom_hdr[('2001','1022')].value
                        EPI_factor = dicom_hdr[('2001','1013')].value
                        ReconMatrixPE = dicom_hdr[('0028','0010')].value
                        ImagingFrequency = data.get('ImagingFrequency')
                        EES = WFS/(ImagingFrequency*(EPI_factor+1)*3.4)                     
                        TotalReadOutTime = EES * (ReconMatrixPE - 1) 
                        PhaseEncodingDirection = data['PhaseEncodingAxis']
                    else: #SIEMENS scanners
