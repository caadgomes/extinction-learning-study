...
trim_values = [2,None] #if no trim then set trim_values = None
desclab = 'trimmed{}'.format(''.join(map(str,trim_values)))

AG = 'A03'
exp = '3T'

bids_dir = os.path.join('/home/data', AG, exp) #level 0

wf_dir = os.path.join(bids_dir, 'workflows') #level 1
rawdata_dir = os.path.join(bids_dir, 'rawdata') #level 1
source_dir = os.path.join(bids_dir, 'sourcedata', 'niftis') #level 1
work_dir = os.path.join(bids_dir, 'work_dir', 'func_denoised') #level 1
derivatives_dir = os.path.join(bids_dir, 'derivatives') #level 1
fmriprep_dir = os.path.join(derivatives_dir, 'fmriprep') #level 1
freesurfer_dir = os.path.join(fmriprep_dir, 'sourcedata','freesurfer') #level 1
...
templates = {
    ...
    # 'bold_space_MNI152NLin6Asym_aroma': os.path.join(fmriprep_dir,'{ptc}', 'func', '*space-MNI152NLin6Asym_desc-smoothAROMAnonaggr_bold.nii.gz'),
    # masks
...
# a varaince_threshold > 0.5 will fail with:
# ERROR: total number of fixed regressors (208) is too many for 190 retained time points!
...
wf_denoising_rest.connect([\
                ...
                # (infosource, datasink_conf_MNI152NLin6Asym, [('ptc','strip_dir'),
                #                                               ('ptc','container')]),
                # (nuisreg_MNI152NLin6Asym_afni, datasink_conf_MNI152NLin6Asym, [('out_file','confounds.space-MNI152NLin6Asym')]),
...
def create_BIDS_filenames(ptc,denoised_dir=func_denoised_dir):
    ...
    for sf in subfolders:
        ...
        for f in files:
            ...
            all_labs = '_'.join(split_f[:-1]) #exclude ext
