...
trim_values = [2,None] #if no trim then set trim_values = [2, None]
desclab = 'trimmed{}'.format(''.join(map(str,trim_values)))
space = 'T1w'
confounds = 'Satterthwaite'

# do not add * after {conf} as there are confs with different endings: Basic_bold, BasicGlobal_bold, and * would select them all
...
func_denoised_dir = os.path.join(derivatives_dir, 'func_denoised') #level 1
...
FC_ROI_dir = os.path.join(derivatives_dir, 'FC_ROI') #level 1
...
class ConnMatrix(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        # df_corr_up = df_corr.loc[lambda x: x.ROI1 < x.ROI2]
...
class GraphTheory(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        # df_corr = pd.read_csv(self.inputs.df_corr)
        # df_corr = pd.pivot(data=cc, index='ROI1', columns='ROI2').reindex(index=c['ROI1'].unique().reindex(c['ROI2'].unique()))
...
wf_FC_ROI.connect([\
                        ...
                        # (wcoherence, datasink, [('df_dist','{s}.{d}.@f10'.format(s=space,d=name_wf))]),                        
